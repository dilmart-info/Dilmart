...
- Structural validation of payload:
  ...
  - Validates every item: non-empty string `customer_ref`, non-empty string `phone_masked`, non-negative integer `orders`, finite non-negative number `spent`, valid date string `last_order_at`.
...
- **Static Analysis & Builds:**
  ...
  - `npm run build`: SUCCESS (Vite production build clean in ~11.5s).
