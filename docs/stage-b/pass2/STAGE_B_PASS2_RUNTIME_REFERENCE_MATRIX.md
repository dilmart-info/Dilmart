...
| `checkout_attempts.store_cart_id` | Column | **0** | 0 | 8 | **ZERO** (FK dropped first) | Migration E |
| `checkout_attempts.store_linked_profile_id` | Column | **0** | 0 | 8 | **ZERO** (FK dropped first) | Migration E |
...
Before executing future Stage B cleanup migrations, active tests referencing legacy objects must be handled according to the following matrix:
...
| `backend/tests/db-integration/customer-entry-eligibility.test.mjs` | `requires_verified_salon` | Entry criteria test | **REWRITE** | Update test fixture to test modern eligibility rules. |
