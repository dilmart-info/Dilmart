...
export class OrdersService {
  ...
  async getB2BOrders(linkedProfileId: string, limit = 50) {
    const { data, error, count } = await this.supabaseAdmin.client
      .from("orders")
      .select(
        "id, order_number, status, delivery_status, payment_method, payment_status, subtotal, delivery_cost, total, area, created_at, governorates(name)",
        { count: "exact" },
      )
      .eq("store_linked_profile_id", linkedProfileId)
      .eq("channel", "barber_app_checkout")
      .order("created_at", { ascending: false })
      .limit(limit);
    ...
    const orders = (data || []).map((o: any) => ({
      ...
      governorate_name: o.governorates?.name ?? null,
      area: o.area,
  ...
  async getB2BOrderDetail(orderId: string, linkedProfileId: string) {
    const { data: order, error } = await this.supabaseAdmin.client
      .from("orders")
      .select(
        `id, order_number, status, delivery_status, payment_method, payment_status,
         subtotal, delivery_cost, discount, total, customer_name, customer_phone,
         area, nearest_landmark, notes, created_at, store_linked_profile_id,
         governorates(name),
         order_items(id, product_id, product_name, price, quantity, products(images))`,
      )
      .eq("id", orderId)
      .eq("store_linked_profile_id", linkedProfileId)
      .eq("channel", "barber_app_checkout")
    ...
    return {
      order: {
        ...
        governorate_name: (order.governorates as any)?.name ?? null,
        area: order.area,
