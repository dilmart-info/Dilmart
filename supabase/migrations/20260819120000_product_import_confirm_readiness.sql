-- Enforce the authoritative product readiness invariant inside product_import_confirm_atomic
-- (DilMart-STORE-PRODUCT-READINESS-INVARIANT-001).
--
-- Same signature/body as 20260802140100_product_import_confirm_short_description.sql, plus one
-- addition to the Phase 2.5 payload-integrity pre-pass: a row whose EFFECTIVE state would be
-- active / published / public must satisfy the same readiness rules ProductsService enforces on
-- every other activation path (name, slug, category, price > 0, stock >= 0, valid discount, at
-- least one image, non-empty description), and the published/public flags require is_active.
--
-- The NestJS import preview already rejects such rows before a session is ever stored; this is
-- the independent DB-level gatekeeper for a stale (pre-deploy) or tampered preview payload, in
-- the same spirit as the existing draft-merchant safety net. Rows for a non-active merchant are
-- exempt because Phase 3 already force-overrides them to inactive/unpublished/private.
--
-- Failure mode: raises IMPORT_ROW_NOT_READY, the whole transaction rolls back, zero product
-- writes, the session returns to 'previewed' and can be re-previewed after fixing the CSV.
...
DECLARE
  ...
  -- Readiness invariant (this migration).
  v_chk_is_active       BOOLEAN;
  v_chk_is_published    BOOLEAN;
  v_chk_name            TEXT;
  v_chk_slug            TEXT;
  v_chk_description     TEXT;
...
BEGIN
  ...
  LOOP
    ...
    -- Readiness invariant: a row that would land active/published/public must pass the same
    -- activation readiness rules as every other product path. Skipped when the merchant is not
    -- active, because Phase 3 forces those rows to inactive/unpublished/private regardless.
    v_chk_is_active    := COALESCE((v_normalized->>'is_active')::boolean, false);
    v_chk_is_published := COALESCE((v_normalized->>'is_published')::boolean, false);
    IF NOT v_force_draft_safety AND (v_chk_is_active OR v_chk_is_published OR v_chk_visibility = 'public') THEN
      IF NOT v_chk_is_active THEN
        RAISE EXCEPTION 'IMPORT_ROW_NOT_READY: row % sku % is_published/public requires is_active', v_chk_row_number, v_chk_sku;
      END IF;

      v_chk_name := btrim(COALESCE(v_normalized->>'name', ''));
      IF v_chk_name = '' THEN
        RAISE EXCEPTION 'IMPORT_ROW_NOT_READY: row % sku % missing name', v_chk_row_number, v_chk_sku;
      END IF;

      v_chk_slug := btrim(COALESCE(v_normalized->>'slug', ''));
      IF v_chk_slug = '' THEN
        RAISE EXCEPTION 'IMPORT_ROW_NOT_READY: row % sku % missing slug', v_chk_row_number, v_chk_sku;
      END IF;

      v_chk_description := btrim(COALESCE(v_normalized->>'description', ''));
      IF v_chk_description = '' THEN
        RAISE EXCEPTION 'IMPORT_ROW_NOT_READY: row % sku % missing description', v_chk_row_number, v_chk_sku;
      END IF;

      IF v_chk_image = '' THEN
        RAISE EXCEPTION 'IMPORT_ROW_NOT_READY: row % sku % missing image', v_chk_row_number, v_chk_sku;
      END IF;

      IF v_chk_visibility = 'archived' THEN
        RAISE EXCEPTION 'IMPORT_ROW_NOT_READY: row % sku % cannot be active and archived', v_chk_row_number, v_chk_sku;
      END IF;
    END IF;
  END LOOP;
