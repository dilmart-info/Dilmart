-- Fix merchant payout order settlement_status consistency.
--
-- Root cause (R1): create_payout_batch_atomic advanced merchant_ledger_entries
-- to 'in_payout' but did NOT update orders.settlement_status.  This left the
-- orders table showing 'payable' when the entries were already locked in a batch.
--
-- Fix: add a fifth phase to the RPC that updates orders.settlement_status to
-- 'in_payout' for all orders whose ledger entries were just added to the batch.
-- The WHERE guard (settlement_status = 'payable') ensures we never regress
-- orders that are already further along (e.g. 'settled', 'disputed').
...
BEGIN
  ...
  -- Early return if no eligible entries.
  ...
  -- Phase 5 (NEW): Advance orders.settlement_status to 'in_payout'.
  -- Only touches orders currently at 'payable' to avoid regressing
  -- 'settled' or 'disputed' orders.
  UPDATE public.orders
  SET    settlement_status = 'in_payout',
         updated_at        = now()
  WHERE  id IN (
    SELECT DISTINCT mle.order_id
    FROM   public.merchant_ledger_entries mle
    WHERE  mle.payout_batch_id = v_batch_id
      AND  mle.order_id IS NOT NULL
  )
  AND settlement_status = 'payable';

  -- Fetch full batch row for the return payload.
