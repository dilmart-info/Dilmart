-- Fix: Drop ALL overloaded place_order functions, then create the single correct version.
-- This resolves:
--   1. "function min(uuid) does not exist" — old overloads used MIN(p.merchant_id) on uuid
--   2. PostgreSQL choosing the wrong overload due to ambiguous default-parameter matching
--   3. "column updated_at of relation products does not exist"

-- Ensure updated_at column exists on products (referenced by place_order)
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();

-- Drop all 4 existing overloads by their exact signatures
DROP FUNCTION IF EXISTS public.place_order(TEXT, TEXT, UUID, TEXT, TEXT, TEXT, NUMERIC, NUMERIC, NUMERIC, NUMERIC, UUID, JSONB, UUID, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, INTEGER, NUMERIC, INTEGER, UUID);

DROP FUNCTION IF EXISTS public.place_order(TEXT, TEXT, UUID, TEXT, TEXT, TEXT, NUMERIC, NUMERIC, NUMERIC, NUMERIC, UUID, JSONB, UUID, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, INTEGER, NUMERIC, INTEGER, UUID, TEXT, TEXT, NUMERIC, NUMERIC, NUMERIC, TEXT, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, INTEGER);
